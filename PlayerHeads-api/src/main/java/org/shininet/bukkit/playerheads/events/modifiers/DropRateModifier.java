...
public class DropRateModifier {
     ...
     * Note: this only provides a useful result with modifiers of type ADD_MULTIPLE_PER_LEVEL, otherwise it will return 1.
     ...
     * Note: only other multipliers can be converted.
     * @return the equivalent additive multiplier value, or null.
     */
    public Double getAdditiveMultiplierValue(){
        if(type==DropRateModifierType.MULTIPLY) return value-1.0;
        if(type==DropRateModifierType.ADD_MULTIPLE) return value;
        if(type==DropRateModifierType.ADD_MULTIPLE_PER_LEVEL) return value*level;
        return null;
    }
    ...
    public Double getMultiplierValue(){
        if(type==DropRateModifierType.MULTIPLY) return value;
        if(type==DropRateModifierType.ADD_MULTIPLE) return 1.0+value;
        if(type==DropRateModifierType.ADD_MULTIPLE_PER_LEVEL) return 1.0+value*level;
        return null;
    }
    
    /**
     * Apply the droprate modifier to the droprate value
     * @param droprate a droprate between 0 (0% chance) and 1.0 (100% chance). Higher values also indicate 100% chance.
