...
public class DropManager {
    ...
    public static void scheduleItemDrops(JavaPlugin plugin, List<ItemStack> drops, final Location location, final boolean naturally, final boolean delayDrop, final long tickDelay) {
    ...
    public static void requestNewDrops(JavaPlugin plugin, List<ItemStack> drops, boolean isWitherDrop, @NotNull Location location) {
        if (drops.isEmpty()) {
            return;
        }
        if (isWitherDrop && plugin.getConfig().getBoolean("delaywitherdrop")) {
            int delay = plugin.getConfig().getInt("delaywitherdropms");
    ...
    public static void requestNewDrops(JavaPlugin plugin, List<ItemStack> drops, boolean isWitherDrop, @NotNull BukkitLocatable locatable) {
        requestNewDrops(plugin, drops, isWitherDrop, locatable.getLocation());
    }

    public static void requestNewDrops(JavaPlugin plugin, List<ItemStack> drops, boolean isWitherDrop, @NotNull BukkitLocatableEvent event) {
        requestNewDrops(plugin, drops, isWitherDrop, event.getLocation());
    }

    public static void requestDrops(JavaPlugin plugin, List<ItemStack> drops, boolean isWitherDrop, @NotNull EntityDeathEvent event) {
        if (drops.isEmpty()) {
            return;
        }
        if (event == null) {
        ...
        boolean needDelayedWitherDrop = isWitherDrop && plugin.getConfig().getBoolean("delaywitherdrop");
        boolean needDelayedDrop = needDelayedWitherDrop || plugin.getConfig().getBoolean("antideathchest");
        if (needDelayedDrop) {
    ...
    public static ItemStack createConvertedMobhead(JavaPlugin plugin, PHHeadType skullType, boolean isSourceSkinnable, boolean addLore, int quantity) {
        boolean usevanillaskull = plugin.getConfig().getBoolean("dropvanillaheads");
        boolean convertvanillahead = plugin.getConfig().getBoolean("convertvanillaheads");
    ...
    public static BlockDropResult blockDrop(JavaPlugin plugin, BlockEvent event, Block block, BlockState state) {
